...
public:
    ...
    map<unsigned int, cv::Point2f> prev_id_pts;

    vector<cv::Point2f> curr_points;
    vector<cv::Point2f> curr_un_points;
    map<unsigned int, cv::Point2f> curr_id_pts;

    vector<unsigned int> ids;

    vector<cv::Point2f> right_points;
    vector<cv::Point2f> right_un_points;
    map<unsigned int, cv::Point2f> right_prev_id_pts;
    map<unsigned int, cv::Point2f> right_curr_id_pts;
...
public:
    ...
    void setOutputFeature(AllInstancesFeature &result);
    void addViodeInstancesBySegImg(SegImage &img);
...
private:
    ...
    static void ptsVelocity(double dt,vector<unsigned int> &ids,vector<cv::Point2f> &curr_un_pts,map<unsigned int, cv::Point2f> &prev_id_pts,
                     map<unsigned int, cv::Point2f> &output_cur_id_pts,vector<cv::Point2f> &output_velocity);
