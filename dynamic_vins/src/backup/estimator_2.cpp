...
void Estimator::inputSemanticImage(SegImage &img)
{
    inputImageCnt++;
    ///进行特征的检测和跟踪
    featureTracker.insts_tracker->vel_map = insts_manager.getInstancesVelocity();
    FeatureMap features = featureTracker.trackSemanticImage(img);
    InstanceFeatureMap instances;
    featureTracker.insts_tracker->setOutputFeature(instances);

    static int last_cnt=inputImageCnt;

    ///发布跟踪可视化图像
    if (Config::SHOW_TRACK){
        cv::Mat imgTrack = featureTracker.getTrackImage();
        pubTrackImage(imgTrack, img.time0);

        /*        string label=to_string(img.time0)+".jpg";
                if(saved_name.count(label)!=0){
                    cv::imwrite(label,imgTrack);
                }*/
        /*        for(auto &inst : featureTracker.insts_tracker->instances){
                    if((std::abs(inst.second.vel.x())>100 || std::abs(inst.second.vel.y())>100) &&
                    inst.second.visual_points_pair.size()>30 &&
                    inputImageCnt-last_cnt>10
                    ){
                        cv::imwrite(to_string(img.time0)+".jpg",imgTrack);
                        last_cnt=inputImageCnt;
                        break;
                    }
                }*/
    }
    if(inputImageCnt % 2 == 0){
        mBuf.lock();
        featureBuf.emplace(std::move(features),img.time0);//放入特征队列中
        instancesBuf.push(instances);
        mBuf.unlock();
    }
}


void Estimator::inputImageNaive(SegImage &img)
{
    inputImageCnt++;

    FeatureMap features = featureTracker.trackImageNaive(img);
    ...
    {
        mBuf.lock();
        featureBuf.emplace(std::move(features),img.time0);
        mBuf.unlock();
    }

}

void Estimator::inputImage(SegImage &img)
{
    inputImageCnt++;

    FeatureMap features = featureTracker.trackImage(img);
    //printf("featureTracker time: %f\n", featureTrackerTime.toc());
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    {
        marginalization_flag = MarginFlag::MARGIN_SECOND_NEW;
        //printf("non-keyframe\n");
    }
