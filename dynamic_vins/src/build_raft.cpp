...
void TestModel(const string& path){
    ...
    DebugS("ForwardFNet dim0:{}", DimsToStr(dim0));
    ...
    DebugS("ForwardFNet dim1:{}", DimsToStr(dim1));
...
int main(int argc, char **argv)
{
    ...
    try{
        ...
        sg_logger->critical(e.what());
        cerr<<e.what()<<endl;
        return -1;
    }

    fmt::print("start build fnet_onnx_path\n");
    if(Build(Config::kRaftFnetOnnxPath,Config::kRaftFnetTensorrtPath)!=0){
        return -1;
    }
    fmt::print("start build cnet_onnx_path\n");
    if(Build(Config::kRaftCnetOnnxPath,Config::kRaftCnetTensorrtPath)!=0){
        return -1;
    }
    fmt::print("start build update_onnx_path\n");
    if(Build(Config::kRaftUpdateOnnxPath,Config::kRaftUpdateTensorrtPath)!=0){
        return -1;
    }
