...
void LineFeatureTracker::NearbyLineTracking(
        const vector<Line>& forw_lines, const vector<Line>& cur_lines,
...
void visualize_line_match(const Mat& imageMat1, const Mat& imageMat2,
    ...
    drawLineMatches( img1, octave0_1, img2, octave0_2, good_matches, lsd_outImg, Scalar::all( -1 ),
                     Scalar::all( -1 ), lsd_mask,DrawLinesMatchesFlags::DEFAULT );
    //    srand(time(NULL));
...
void visualize_line(const Mat& imageMat1,std::vector<KeyLine> octave0_1)
{
    //	Mat img_1;
    cv::Mat img1;
    if (imageMat1.channels() != 3){
...
/*     opts.refine       = 1;
	...
	//void LSDDetectorC::detect( const std::vector<Mat>& images, std::vector<std::vector<KeyLine> >& keylines,
	// int scale, int numOctaves, const std::vector<Mat>& masks ) const
    lsd_->detect( img, lsd, 2, 1, opts);

    cout<<"detect:"<<lsd.size()<<endl;
...
/*     visualize_line(img,lsd);
    ...
    cout<<"forwframe_->lineID.push_back"<<endl;


    /// 线特征匹配
    if(!curframe_->keylsd.empty()){
        /* compute matches */
