...
FrameLines::Ptr LineDetector::Detect(cv::Mat &img,const cv::Mat &mask) {
    ...
    for ( int i = 0; i < (int) lsd_temp.size(); i++ ){
        if( lsd_temp[i].octave == 0 && lsd_temp[i].lineLength >= 60 ){
            if(!mask.empty()){
                //根据mask剔除
                if(mask.at<uchar>(lsd_temp[i].getStartPoint())>0.5 && mask.at<uchar>(lsd_temp[i].getEndPoint())>0.5){
...
void LineDetector::VisualizeLineStereoMatch(cv::Mat &img, const FrameLines::Ptr &left_lines,
...
void LineDetector::VisualizeLineMonoMatch(cv::Mat &img, const FrameLines::Ptr &prev_lines,
                                          const FrameLines::Ptr &curr_lines){
